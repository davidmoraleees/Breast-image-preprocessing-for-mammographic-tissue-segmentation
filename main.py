...
def separate_periphery(image): # Function to separate the breast periphery using thresholding and morphological operations
    ...
    bpa_combined = np.logical_or(bpa_otsu, bpa_threshold) # Combine the two thresholding methods to get a better separation 
    bpa_filled = morphology.remove_small_holes(bpa_combined, area_threshold=64) # Fill small holes in the binary image
    ...
    largest_region = max(regions, key=lambda r: r.area) # Find the largest region, which should correspond to the breast
